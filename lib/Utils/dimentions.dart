import 'package:get/get.dart';

// Emulator Screen Size H:813 W:392
class Dimentions {
  static double screenHeight = Get.height;
  static double screenWidth = Get.width;
  static double pageView = screenHeight / 2.59;
  static double pageViewContainer = screenHeight / 3.69;
  static double pageViewTextContainer = screenHeight / 6.75;
  // Dynamic height padding and margin
  static double height10 = screenHeight / 81.3;
  static double height15 = screenHeight / 54.2;
  static double height20 = screenHeight / 40.6;
  static double height30 = screenHeight / 27.1;
  static double height40 = screenHeight / 20.32;
  static double height45 = screenHeight / 18.06;
  static double height100 = screenHeight / 8.13;
  static double height120 = screenHeight / 6.77;

  // Dynamic width padding and margin
  static double width10 = screenHeight / 81.3;
  static double width15 = screenHeight / 54.2;
  static double width20 = screenHeight / 40.6;
  static double width30 = screenHeight / 27.1;
  static double width100 = screenHeight / 8.13;
  static double width120 = screenHeight / 6.77;

  // Font
  static double font16 = screenHeight / 50.81;
  static double font18 = screenHeight / 45.16;
  static double font20 = screenHeight / 40.6;
  static double font26 = screenHeight / 31.26;

  // Radius
  static double radius15 = screenHeight / 54.2;
  static double radius20 = screenHeight / 40.6;
  static double radius30 = screenHeight / 27.1;

  // Icon size
  static double iconSize16 = screenHeight / 50.81;
  static double iconSize24 = screenHeight / 33.87;

  // ListView size 813 x 392
  static double listViewImgSize = screenWidth / 3.26;
  static double listViewTextContSize = screenWidth / 3.92;

  // Popular food
  static double popularFoodImgSize = screenHeight / 2.32;

  // Bottom height
  static double bottomHeightBar120 = screenHeight / 6.77;

  // SplashPage
  static double splashImg = screenHeight / 3.38;
}
